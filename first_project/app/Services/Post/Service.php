...
class Service
{
    public function store($data)
    {
        try {
            ...
            $post->tags()->attach($tagIds); // продолжаем sql-запрос и создаем связи с тегами
    ...
    public function update($post, $data)
    {
        try {
            ...
            $post->update($data);
            $post->tags()->sync($tagIds); // удаляет старые связи и создает новые
            DB::commit();
        } catch (\Exception $exception) {
            DB::rollBack();
            return $exception->getMessage();
    ...
    private
    function getTagIdsWithUpdate($tags)
    {
        $tagIds = [];
        foreach ($tags as $tag) {
            if (!isset($tag['id'])) {
